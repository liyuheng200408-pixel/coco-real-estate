...
**业主/租客要求（2026-08-29 加）**：①录入房源时**顺带填写业主信息**（`owner_name`/`owner_phone`/`owner_wechat`，电话加密）→ 系统自动登记房东并关联此房源（owner_id）；②出租房源填**租客要求**（`tenant_requirements`，如不吸烟/办居住证/学生优先）→ 正式入库，匹配租客时用于过滤（否定要求"不X"与客户资料冲突则排除）。反馈：填了业主信息就确认"已登记关联"，没填才中性提示"未录入，可后续补充"，**禁止**说"可在房源详情中补充"这种让用户以为没录进去的话。
