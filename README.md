...
1. **数据安全与备份**：Coco 房产机器人会将业务数据（客户信息、房源、跟进、成交等）存储在你指定的数据库中。请务必**定期备份数据**（项目提供 `backup_db.py` 备份工具及每日自动备份），并妥善保管备份文件。因未备份、误删、重装系统、服务器故障等造成的数据丢失，项目方不承担责任。

2. **加密密钥保管**：客户手机号、微信号等敏感字段使用加密密钥（COCO_ENC_KEY）加密存储。**该密钥是解密客户数据的唯一凭证**，一旦丢失或泄露，将导致客户数据永久无法解密。请务必独立备份该密钥，因密钥丢失造成的数据不可读，项目方不承担责任。

3. **软件按现状提供**：本项目基于 Hermes Agent 深度定制，按"现状"（AS-IS）提供，不承诺无缺陷、不保证特定功能完全满足你的需求。使用过程中出现的任何问题或损失，项目方不承担由此产生的直接或间接责任。

4. **数据准确性**：贷款政策、税费等外部信息具有时效性，Coco 查询到的政策信息仅供参考，请以当地官方机构的最新公告为准。因政策信息滞后或不准确造成的损失，项目方不承担责任。

5. **合规使用**：请确保你的业务操作符合当地法律法规（如个人信息保护法、房地产中介管理规定等）。因违规使用本软件产生的法律后果，由使用者自行承担。
