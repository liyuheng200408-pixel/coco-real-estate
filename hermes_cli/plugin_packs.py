...
def render_pack_review(console, pack: PluginPack, resolved: List[ResolvedPackPlugin]) -> None:
    ...
    if pack.skills:
        ...
        console.print("[dim]Install them manually, e.g. `coco skills install <id>`.[/dim]")
    console.print(
...
def cmd_pack_show(source: str) -> None:
    ...
    console.print("\n[dim]Dry run only. Install with `coco plugins pack install ...`.[/dim]")
...
def cmd_pack_install(source: str, *, force: bool = False) -> None:
    ...
    if ok:
        ...
        console.print("[dim]  coco gateway restart[/dim]")
    if failed:
        sys.exit(1)
