...
_TTS_SUMMARY_ROWS = {
    ...
    "neutts": ("NeuTTS", "neutts", "run 'coco setup tts'"),
    "kittentts": ("KittenTTS", "kittentts", "run 'coco setup tts'")}
...
_STT_SUMMARY_DEFAULT = ("Local Whisper", "faster_whisper", "run 'coco tools' → Speech-to-Text")
...
_EDIT_WIZARD_ROWS = (
    ("coco setup", "            Re-run the full wizard"), ("coco setup model", "      Change model/provider"),
    ("coco setup terminal", "   Change terminal backend"), ("coco setup gateway", "    Configure messaging"),
    ("coco setup tools", "      Configure tool providers"))
_EDIT_CONFIG_ROWS = (
    ("coco config", "           View current settings"), ("coco config edit", "      Open config in your editor"),
    ("coco config set <key> <value>", ""))
_READY_ROWS = (
    ("coco status", "           Service status"), ("coco logs", "             Follow the service log"),
    ("coco check", "            Deployment health check"))
...
def _vision_row(config, feats):
    ...
    try:
        ...
        ok = False
    return ("Vision (image analysis)", ok, None if ok else "run 'coco setup' to configure")
...
def _modal_row(config, feats):
    ...
    if _setup.cfg_get(config, "terminal", "backend") == "modal":
        ...
        return ("Modal Execution", False, "run 'coco setup terminal'")
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    try:
        ...
        _setup._info("  Finish this one step with either of:",
              "    coco model              (pick any provider/model)",
              "    coco setup --portal     (Nous Portal OAuth, no API key)")
    ...
    if available_count < len(tool_status):
        _setup.print_warning("Some tools are disabled. Run 'coco setup tools' to configure them,")
