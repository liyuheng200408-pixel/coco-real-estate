    ...
    const session = new VoiceLiveSession({
      ...
      onDelegation: (delegationId, context) => {
        ...
        void Promise.resolve(latest.current.onSubmit(prompt, voiceContext)).catch(error => {
          ...
          session.speak(delegationId, 'Sorry, I could not reach Coco for that request.')
          setDelegation(null)
          refreshStatus()
        })
      },
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (tool && tool !== lastToolLabelRef.current) {
        ...
        session.think(delegationId, `Coco is working: ${tool}. Not done yet.`)
      }
      ...
      if (
        ...
        if (spokenLengthRef.current === 0) {
          session.think(delegationId, 'Coco finished that request without a spoken result.')
        }
