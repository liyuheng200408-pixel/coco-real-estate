...
def _snapshot_primary_runtime(agent):
    ...
    # Coco Real Estate DB initialization
    # （同步上游时需重新应用，见 patches/04-agent-init-db.patch）
    # 说明：gateway 启动路径不经过 init_agent，靠 real_estate_db.get_real_estate_db() 惰性建表兜底；
    #       两条路都要在，否则新装实例的库可能没有表。
    from agent.real_estate_db import init_real_estate_db
    db_url = os.getenv("DATABASE_URL", "sqlite:///real_estate.db")
    try:
        init_real_estate_db(db_url)
        if not agent.quiet_mode:
            print("[Coco] 房产数据库初始化完成")
    except Exception as e:
        if not agent.quiet_mode:
            print(f"[Coco] 房产数据库初始化失败: {e}")
