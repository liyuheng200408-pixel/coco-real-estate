...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    # Coco Real Estate guidance — always inject for this fork
    # （同步上游时需重新应用，见 patches/03-system-prompt-inject-prompt.patch；
    #   必须留在 stable 片段里，拼进 volatile 会破坏 prompt 缓存）
    from agent.real_estate_prompt import get_real_estate_prompt
    stable_parts.append(get_real_estate_prompt())
