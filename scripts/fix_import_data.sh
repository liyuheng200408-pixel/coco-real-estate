...
echo "== 3. 出租房源价格 元/月 → 万元 =="
psql "$DB_URL" -c "UPDATE re_properties SET price = round(price/10000.0, 4), unit_price = round(unit_price/10000.0, 2) WHERE property_type='rental' AND price > 100;"

echo "== 4. 修正出租 unit_price（按新 price 重算，元/㎡） =="
psql "$DB_URL" -c "UPDATE re_properties SET unit_price = CASE WHEN area > 0 THEN round(price*10000.0/area, 0)::int ELSE unit_price END WHERE property_type='rental';"

echo "== 修复后校验 =="
psql "$DB_URL" -t -c "SELECT property_type, min(price), max(price), round(avg(price),2) FROM re_properties GROUP BY property_type;"
